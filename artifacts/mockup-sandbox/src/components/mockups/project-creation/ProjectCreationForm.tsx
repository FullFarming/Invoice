...
function serviceLabel(svc: ServiceItem) {
  return `${svc.category} (${svc.workdayProductCode})`;
}
  ...
  return (
    <nav className="w-full bg-white border-b border-gray-200 px-6 h-14 flex items-center justify-between shrink-0">
      <div className="flex items-center gap-3">
        ...
        <img src="/__mockup/images/cw-logo-color2.png" alt="Cushman & Wakefield" className="h-8 object-contain" />
      </div>
      <div className="flex items-center gap-3">
...
function NavButtons({ step, onBack, onNext, canNext, nextLabel = "다음 단계" }: {
  ...
  return (
    <div className="flex items-center justify-between mt-10">
      {step > 0 ? (
        <button onClick={onBack} className="flex items-center gap-1.5 px-5 py-2.5 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
          <ChevronLeft className="w-4 h-4" />이전 단계
        </button>
      ) : <div />}
      <button onClick={onNext} disabled={!canNext} className={`flex items-center gap-1.5 px-6 py-2.5 text-sm font-medium rounded-lg ${canNext ? "bg-[#1a2744] text-white hover:bg-[#253561]" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <InfoHint>프로젝트명이 자동 생성에 사용됩니다.</InfoHint>
                </div>

                <div>
                  <FieldLabel required>Broad Category</FieldLabel>
                  <Select
                    value={step1.broadCategory}
                    onValueChange={(v) => handleStep1Change({ broadCategory: v as BroadCategory })}
                  >
                    <SelectTrigger className="h-10 text-sm border-gray-300 bg-white">
              ...
              {/* Row 2: Service (full width, cascaded) */}
              <div>
                <FieldLabel required>Service</FieldLabel>
                <Select
                  value={step1.serviceId}
                  onValueChange={(v) => handleStep1Change({ serviceId: v })}
                  disabled={!step1.broadCategory}
                >
          ...
          <div>
            ...
            <div className="flex gap-6">
              <div className="flex-1 flex flex-col gap-5">
                ...
                <label className="flex items-center gap-3 px-4 py-3.5 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
          ...
          <div>
            ...
            <div className="flex flex-col gap-6">
              ...
              <div>
                ...
                <div className="border border-dashed border-gray-300 rounded-lg p-8 flex flex-col items-center gap-2 cursor-pointer hover:border-gray-400 hover:bg-gray-50 text-center">
                  <div className="w-9 h-9 rounded-md bg-blue-50 flex items-center justify-center">
                    <Upload className="w-4 h-4 text-blue-500" />
                  </div>
                  <span className="text-xs text-gray-500">클릭하거나 파일을 드래그하여 업로드하세요.</span>
