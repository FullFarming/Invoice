...
function generateProjectName(step1: Step1Data): string {
  ...
  if (step1.role) parts.push(step1.role);
  if (step1.service) {
    ...
    parts.push(svcMap[step1.service] || step1.service);
  }
    ...
    <div className="flex items-center">
      {steps.map((label, i) => (
        <div key={i} className="flex items-center">
          ...
          {i < steps.length - 1 && (
            <div
              className={`h-px w-40 mb-5 mx-1 ${
                i < current ? "bg-[#1a2744]" : "bg-gray-200"
              }`}
            />
          )}
        </div>
  ...
  return (
    <div className="flex items-center justify-between mt-10">
      {step > 0 ? (
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 px-5 py-2.5 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
        >
      ...
      <button
        onClick={onNext}
        disabled={!canNext}
        className={`flex items-center gap-1.5 px-6 py-2.5 text-sm font-medium rounded-lg transition-colors ${
          canNext
            ? "bg-[#1a2744] text-white hover:bg-[#253561]"
    ...
    <div className="flex flex-col gap-5 pb-2">
      <div className="pt-1">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <FieldLabel>{dealLabel}</FieldLabel>
            <CurrencyAmountInput
...
function SummaryCard({ step1, step2 }: { step1: Step1Data; step2: Step2Data }) {
  const svcLabel = [step1.role, step1.service].filter(Boolean).join("_") || "—";
  return (
    <div className="bg-[#eef1f9] rounded-xl p-5 flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-2.5">
        <div className="flex justify-between items-start gap-2">
          <span className="text-xs text-gray-500 shrink-0">Role & Service</span>
          <span className="text-xs font-medium text-gray-800 text-right">{svcLabel}</span>
        </div>
        <div className="flex justify-between items-start gap-2">
          ...
          <>
            ...
            {step2.serviceDetails.deposit && (
              <div className="flex justify-between items-start gap-2">
                ...
                <span className="text-xs font-medium text-gray-800 text-right">
                  {step2.serviceDetails.depositCurrency} {step2.serviceDetails.deposit}
                </span>
              </div>
            )}
            {step2.serviceDetails.monthlyRent && (
              <div className="flex justify-between items-start gap-2">
                ...
                <span className="text-xs font-medium text-gray-800 text-right">
                  {step2.serviceDetails.monthlyRentCurrency} {step2.serviceDetails.monthlyRent}
                </span>
              </div>
            )}
          </>
        ...
        {step1.service === "Investment Sales" && (
          <>
            {step2.serviceDetails.areaM2 && (
              <div className="flex justify-between items-start gap-2">
                <span className="text-xs text-gray-500 shrink-0">면적</span>
            ...
            {step2.serviceDetails.dealPrice && (
              <div className="flex justify-between items-start gap-2">
                ...
                <span className="text-xs font-medium text-gray-800 text-right">
                  {step2.serviceDetails.dealPriceCurrency} {step2.serviceDetails.dealPrice}
                </span>
              </div>
            )}
          </>
...
export function ProjectCreationForm() {
  ...
  const updateDetails = (partial: Partial<ServiceDetails>) => {
    setStep2((prev) => ({
      ...prev,
      serviceDetails: { ...prev.serviceDetails, ...partial },
    }));
  };
  ...
  const canProceedStep1 =
    !!step1.clientName && !!step1.buildingAlias && !!step1.role && !!step1.service && !!step1.contractDate;
  ...
  const isOther = !isLeasing && !isInvestment && !!step1.service;

  const dateDisplay = step1.contractDate.replace(/-/g, ".");
            ...
            <div className="flex flex-col gap-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <Input
                    className="h-10 text-sm border-gray-300 pl-9"
                    placeholder="고객사를 검색하거나 새로 입력하세요"
                    value={step1.clientName}
                    onChange={(e) => handleStep1Change({ clientName: e.target.value })}
                  />
                </div>
                <p className="text-xs text-gray-400 mt-1.5 flex items-center gap-1">
                  <span className="inline-block w-3.5 h-3.5 rounded-full border border-gray-400 flex items-center justify-center text-[9px] leading-none font-bold shrink-0">i</span>
                  이미 등록된 고객사를 검색하거나 새로 추가할 수 있습니다.
                </p>
              </div>
              ...
              <div>
                ...
                <Input
                  className="h-10 text-sm border-gray-300"
                  placeholder="예: SFC, Gangnam Finance Center, ABC Tower 12F"
                  value={step1.buildingAlias}
                  onChange={(e) => handleStep1Change({ buildingAlias: e.target.value })}
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select value={step1.service} onValueChange={(v) => handleStep1Change({ service: v as Service })}>
                    <SelectTrigger className="h-10 text-sm border-gray-300 bg-white">
                      <SelectValue placeholder="선택하세요" />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <Input
                      type="date"
                      className="h-10 text-sm border-gray-300 pr-9"
                      value={step1.contractDate}
                      onChange={(e) => handleStep1Change({ contractDate: e.target.value })}
                    />
                  </div>
                  <p className="text-xs text-gray-400 mt-1.5 flex items-center gap-1">
                    <span className="inline-block w-3.5 h-3.5 rounded-full border border-gray-400 flex items-center justify-center text-[9px] leading-none font-bold shrink-0">i</span>
                    날짜는 작성 날짜 기준으로 자동 선택됩니다.
                  </p>
                </div>
                <div>
                  ...
                  <div className="relative">
                    <Input
                      className="h-10 text-sm border-gray-300 pr-8"
                      placeholder="공동 관리자 이름 또는 이메일을 입력하세요"
                      value={step1.coManagers}
                      onChange={(e) => handleStep1Change({ coManagers: e.target.value })}
                    />
              ...
              <div className="flex-1 flex flex-col gap-5">
                <div>
                  ...
                  <Input
                    className="h-10 text-sm border-gray-300"
                    placeholder="예: 350.5"
                    value={step2.serviceDetails.areaM2}
                    onChange={(e) => updateDetails({ areaM2: e.target.value })}
                  />
                </div>

                {isLeasing && (
                  <LeasingServiceFields data={step2.serviceDetails} onChange={updateDetails} />
                )}
                {isInvestment && (
                  <InvestmentSalesFields
                    data={step2.serviceDetails}
                    role={step1.role}
                    onChange={updateDetails}
                  />
                )}
                {isOther && (
                  <OtherServiceFields data={step2.serviceDetails} onChange={updateDetails} />
                )}

                <div>
                  <FieldLabel required>프로젝트 총 금액</FieldLabel>
                  <CurrencyAmountInput
                    value={step2.projectFee}
                    currency={step2.currency}
                    onChange={(v) => setStep2((p) => ({ ...p, projectFee: v }))}
                    onCurrencyChange={(v) => setStep2((p) => ({ ...p, currency: v }))}
                  />
                  <p className="text-xs text-gray-400 mt-1.5 flex items-center gap-1">
                    <span className="inline-block w-3.5 h-3.5 rounded-full border border-gray-400 flex items-center justify-center text-[9px] leading-none font-bold shrink-0">i</span>
                    계약서 상의 전체 금액이며 인보이스는 이 금액 내에서 분할하여 진행 가능합니다.
                  </p>
                </div>
                ...
                <label className="flex items-center gap-3 px-4 py-3.5 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                  <Checkbox
                    checked={step2.plusOneIncluded}
                    onCheckedChange={(v) =>
                      setStep2((p) => ({ ...p, plusOneIncluded: !!v }))
                    }
                ...
                <label className="flex items-center gap-3 px-4 py-3.5 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                  <Checkbox
                    checked={step2.winAnnounce}
                    onCheckedChange={(v) =>
                      setStep2((p) => ({ ...p, winAnnounce: !!v }))
                    }
                    className="border-gray-300"
                  />
        ...
        {step === 2 && (
          <div>
            ...
            <p className="text-sm text-gray-500 mb-7">
              계약서 등 필수 서류를 첨부하고 프로젝트명을 확정해 주세요.
            </p>
            ...
            <div className="flex flex-col gap-6">
              <div>
                ...
                <Input
                  className="h-10 text-sm border-gray-300"
                  value={projectNameOverride || autoProjectName}
                  onChange={(e) => setProjectNameOverride(e.target.value)}
                />
                <p className="text-xs text-gray-400 mt-1.5 flex items-center gap-1">
                  <span className="inline-block w-3.5 h-3.5 rounded-full border border-gray-400 flex items-center justify-center text-[9px] leading-none font-bold shrink-0">i</span>
                  자동 생성된 프로젝트명을 수정할 수 있습니다.
                </p>
              </div>

              <label className="flex items-center gap-2.5 cursor-pointer">
                <Checkbox
                  checked={refundClause}
                  onCheckedChange={(v) => setRefundClause(!!v)}
                  className="border-gray-300"
                />
              ...
              <div>
                ...
                <div className="border border-dashed border-gray-300 rounded-lg p-8 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-gray-400 hover:bg-gray-50 transition-colors text-center">
                  ...
                  <span className="text-xs text-gray-400">
                    PDF, JPG, PNG 형식의 파일을 최대 5개까지 업로드할 수 있습니다.
                  </span>
                </div>
              </div>
            </div>
